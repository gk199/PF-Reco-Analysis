...
The scan axes are eta and energy, parsed from ntuple names of the form:
  ...
  pfObjectsNtuple_<PF>_SinglePiCloseByE20_eta0p5.root
  pfObjectsNtuple_<PF>_SinglePiPythiaE20_eta0p8.root

The filename-safe tags use ``p`` for the decimal point and ``m`` for a minus
sign. CloseBy and Pythia samples are processed as separate scan families so
identical eta/energy coordinates never overwrite one another.
"""
...
DEFAULT_SAMPLE_TYPES = ["CloseBy", "Pythia"]
DEFAULT_NCL_MAX = 6.0
...
def parse_args():
    ...
    parser.add_argument("--input-dir", "--inputdir", dest="input_dir", default="/eos/user/c/chtong/Public/Rereco/SinglePion_OverSplitting_PF3ns_8_8/",
                        help="Directory containing the ntuple ROOT files.")
    parser.add_argument("--output-dir", "--outputdir", dest="output_dir", default="/eos/user/c/chtong/Public/Rereco/SinglePion_OverSplitting_PF3ns_8_8/",
    ...
    parser.add_argument(
        ...
        help=(
            "Single-pion sample families to process. Supported values are "
            "CloseBy and Pythia. When both are requested, they are placed in "
            "separate sections of the same PDF and ROOT output."
        ),
    )

    parser.add_argument("--eta-values", default=None,
                        help="Comma-separated eta scan values for heatmaps. If omitted, auto-discover from filenames.")
    parser.add_argument("--energy-values", default=None,
                        help="Comma-separated energy scan values for heatmaps. If omitted, auto-discover from filenames.")
    parser.add_argument("--eta-dist-values", default=None,
                        help="Comma-separated eta values for the 3x3 overlay distributions. If omitted, use first 3 eta points.")
    parser.add_argument("--energy-dist-values", default=None,
                        help="Comma-separated energy values for the 3x3 overlay distributions. If omitted, use first 3 energy points.")
...
def canonical_sample_type(value):
    ...
    if normalized not in aliases:
        raise ValueError(
            f"Unknown sample type '{value}'. Supported values: CloseBy, Pythia."
        )
...
def parse_singlepi_filename(basename, prefix, pf_names):
    ...
    Examples:
      pfObjectsNtuple_standardPF_SinglePiCloseByE20_eta0p5.root
      pfObjectsNtuple_seedTimingPF_SinglePiPythiaE35_eta0p8.root
    """
    ...
    match = re.fullmatch(
        r"SinglePi(?P<sample_type>CloseBy|Pythia)"
...
def main():
    ...
    if len(metadata) == 0:
        raise RuntimeError(
            ...
            "pfObjectsNtuple_standardPF_SinglePiCloseByE20_eta0p5.root "
            "or pfObjectsNtuple_seedTimingPF_SinglePiPythiaE35_eta0p8.root."
        )
    ...
    for sample_type in sample_types:
        ...
        sample_label = (
            "CloseByParticleGunProducer (HCAL-surface launch)"
            if sample_type == "CloseBy"
            else "Pythia8PtGun (interaction-point launch)"
        )
        print_summary(
            stats_by_mode,
            pf_names,
