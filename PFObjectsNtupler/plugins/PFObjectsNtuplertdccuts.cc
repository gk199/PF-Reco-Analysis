...
class PFObjectsNtuplertdccuts : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit PFObjectsNtuplertdccuts(const edm::ParameterSet&);
  ~PFObjectsNtuplertdccuts() override {}
...
PFObjectsNtuplertdccuts::PFObjectsNtuplertdccuts(const edm::ParameterSet& iConfig)
...
void PFObjectsNtuplertdccuts::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  // Clear all vectors
  ...
  if (hcalClusters.isValid()) {
    for (const auto& cl : *hcalClusters) {
      if (cl.energy() < 20.0) {clusterIndex++; continue;} // Apply a cut on cluster energy to reduce noise contributions
      // To-Do: count only the clusters that pass the energy cut for indexing
      ...
      if (hbheRechits.isValid()) {
        ...
        for (const auto& rh : *hbheRechits) {
          if (rh.energy() < 2.0) {continue;} // Apply a cut on rechit energy to reduce noise contributions
          ...
          // Cuts on eta and deltaR between cluster and rechit
          ...
          if ( (detid.depth() == 4 && detid.ietaAbs() == 16) || (detid.ietaAbs() > 16) ) {
            // HE rechit
            // Apply TDC cut for HE
            if (SOI_TDC == 62 || SOI_TDC == 63) continue; 
            ...
            // HB rechit
            // Apply TDC cut for HB
            if (SOI_TDC == 3) continue; 
          ...
          // Save the rechit info to hbhe vectors
...
#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(PFObjectsNtuplertdccuts);